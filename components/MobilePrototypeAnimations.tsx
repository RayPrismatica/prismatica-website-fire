...
export default function MobilePrototypeAnimations() {
  const { isPrototypeEnabled } = useMobilePrototype();

  useEffect(() => {
    if (!isPrototypeEnabled) return;

    // Add animations-ready class after a brief delay
    setTimeout(() => {
    ...
    // Observer for initial reveal (happens early)
